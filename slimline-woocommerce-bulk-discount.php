...
if ( !class_exists( 'Slimline_Woo_Bulk_Discount_Plugin_t4m' ) ) {

	class Slimline_Woo_Bulk_Discount_Plugin_t4m {
		...
		public function woocommerce_loaded() {
			...
			if ( get_option( 'woocommerce_t4m_enable_bulk_discounts', 'yes' ) == 'yes' ) {
				...
				add_action( 'woocommerce_product_after_variable_attributes', array( $this, 'variations_discount_fields' ), 99, 3 );
		...
		protected function gather_discount_coeffs() {
			...
			if ( sizeof( $cart->cart_contents ) > 0 ) {
				foreach ( $cart->cart_contents as $cart_item_key => $values ) {
					...
					$this->discount_coeffs[$this->get_actual_id( $_product )]['coeff'] = $this->get_discounted_coeff( $this->get_actual_id( $_product ), $quantity );
		 ...
		 * @param array $variation_data Array of meta, etc. for the variation
		 * @param object $variation Variation post object
		 */
		protected function variations_discount_fields( $loop, $variation_data, $variation ) {

			if ( 'yes' === get_post_meta( $variation->post_parent, '_bulkdiscount_enabled', true ) ) {
				...
				for ( $i = 1; $i <= 5; $i++ ) {
					...
					?>
						...
						<tr>
							<td>
								<label for="<?php printf( 'variable-bulkdiscount-quantity-%1$s-%2$s', $i, $loop ); ?>"><?php _e( 'Bulk Discount Quantity (min)', 'wc_bulk_discount' ); ?> <a class="tips" data-tip="<?php _e( 'Enter the minimal quantity for which the discount applies.', 'wc_bulk_discount' ); ?>" href="#">[?]</a></label>
								<input id="<?php printf( 'variable-bulkdiscount-quantity-%1$s-%2$s', $i, $loop ); ?>" name="variable_bulkdiscount_quantity_<?php echo $i; ?>[<?php echo $loop; ?>]" placeholder="0" type="number" value="<?php echo ( @$variation_data[ "_bulkdiscount_quantity_{$i}" ][ 0 ] ? $variation_data[ "_bulkdiscount_quantity_{$i}" ][ 0 ] : '' ); ?>" />
							</td>

							<td>
								<label for="<?php printf( 'variable-bulkdiscount-discount-%1$s-%2$s', $i, $loop ); ?>"><?php _e( 'Bulk Discount (%)', 'wc_bulk_discount' ); ?> <a class="tips" data-tip="<?php _e( 'Enter the discount in percents (decimals allowed).', 'wc_bulk_discount' ); ?>" href="#">[?]</a></label>
								<input class="wc_input_decimal" id="<?php printf( 'variable-bulkdiscount-discount-%1$s-%2$s', $i, $loop ); ?>" name="variable_bulkdiscount_discount_<?php echo $i; ?>[<?php echo $loop; ?>]" placeholder="0" type="text" value="<?php echo ( @$variation_data[ "_bulkdiscount_discount_{$i}" ][ 0 ] ? $variation_data[ "_bulkdiscount_discount_{$i}" ][ 0 ] : '' ); ?>" />
							</td>
						</tr>
